    ...
    return (
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          <Btn variant="ghost" onClick={onExit}>{t("retour_btn")}</Btn>
          <div style={{ fontFamily: FONT_DISPLAY, fontSize: 19, fontWeight: 700, color: C.navy }}>{t("jeu_abreviations_titre")}</div>
        </div>
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <div style={{ fontSize: 13, color: C.inkSoft, marginBottom: 20, textAlign: "center" }}>{t("jeu_abreviations_intro")}</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <button onClick={() => startMode("normale")} style={{ background: "#fff", border: `1px solid ${C.line}`, borderRadius: 14, padding: 22, textAlign: "left", cursor: "pointer" }}>
              ...
              <div style={{ fontSize: 12.5, color: C.inkSoft }}>{t("mode_normale_desc_abrev")}</div>
            </button>
          ...
          <div style={{ textAlign: "center", marginTop: 18 }}>
            <a href={GLOSSAIRE_URL} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}>
              <Btn variant="ghost">{t("voir_glossaire_btn")}</Btn>
            </a>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth: 520, margin: "0 auto" }}>
      ...
      <div style={{ background: "#fff", border: `1px solid ${C.line}`, borderRadius: 16, padding: 28 }}>
        <div style={{ fontSize: 11.5, color: C.inkSoft, textTransform: "uppercase", letterSpacing: ".04em", marginBottom: 10, textAlign: "center" }}>{t("question_abreviation")}</div>
        <div style={{ fontSize: 30, fontWeight: 700, color: C.navy, textAlign: "center", marginBottom: 24, fontFamily: FONT_MONO }}>{question.acronyme}</div>
          ...
          {question.options.map((opt, i) => {
            ...
            return (
              <button key={i} disabled={!!feedback} onClick={() => answer(i)}
                style={{ background: bg, border: `2px solid ${border}`, color, borderRadius: 12, padding: "12px 14px", fontSize: 13.5, fontWeight: 600, fontFamily: FONT_BODY, cursor: feedback ? "default" : "pointer", textAlign: "left" }}>
                {opt}
              </button>
            );
          })}
